...
## Prerequisites
For this blog post, I assume you already have created a Function App. If you haven't you could clone this <a href="https://github.com/simonagren/AzureADAzureFunction" target="_blank">solution</a> from my Github, and follow the steps in the readme file. 
...
- Select "Platform Features" and then "Authentication/Authorization"
...
Now there should be an "Authentication" link available directly in the overview of the Function App. Either press that Authentication link, or navigate the same way we did in the previous step via "Authentication/Authorization".
- Select Azure AD again from Application Providers -> Manage Application -> Settings -> Properties.
- Enter an App Id URI based on your tenant, mine was: `https://<tenant>.onmicrosoft.com/pnptesting`
...
### Consent with a user from the other tenant
...
- Go into settings, permissions, select permissions and then search for the name of your AD Application from the other tenant. In my case, it's `pnptesting`.
...
- Go into keys, create a new key. Make a note of the `key value` and the `application id` of this app.
...
- Select Oauth 2.0, Azure Active Directory as the Identity provider, enter the client Id and secret of your newly created app.
...
UPDATE:
I added the link to two articles that involve similar things that I'm doing in this post.
...
So I've created a simple SPFx web part [solution](https://github.com/simonagren/SPFxAzureFunction) that you could clone from my Github.

Follow the readme and replace it with your variables in the code. In this web part, we are requesting API Permissions.
...
### Deploy the solution
- I usually right-click in VS Code and choose to reveal in Explorer.
...
- Now you should see all your SharePoint Lists.
