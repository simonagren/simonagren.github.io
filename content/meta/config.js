module.exports = {
  siteTitle: "AgrenPoint - A dev blog", // <title>
  shortSiteTitle: "AgrenPoint - A dev blog", // <title> ending for posts and pages
  siteDescription: "AgrenPoint is development wiht Simon Ågren.",
  siteUrl: "http://simonagren.github.io/",
  pathPrefix: "",
  siteImage: "preview.jpg",
  siteLanguage: "en",
  // author
  authorName: "Simon Ågren",
  authorTwitterAccount: "agrenpoint",
  // info
  headerTitle: "Simon Ågren",
  headerSubTitle: "MCSD SharePoint Applications | Architect",
  // manifest.json
  manifestName: "AgrenPoint - A dev blog",
  manifestShortName: "AgrenPoint", // max 12 characters
  manifestStartUrl: "/index.html",
  manifestBackgroundColor: "white",
  manifestThemeColor: "#666",
  manifestDisplay: "standalone",
  // social
  authorSocialLinks: [
    { name: "github", url: "https://github.com/simonagren" },
    { name: "twitter", url: "https://twitter.com/agrenpoint" },
    { name: "facebook", url: "http://facebook.com/simon.agren52" }
  ]
};
